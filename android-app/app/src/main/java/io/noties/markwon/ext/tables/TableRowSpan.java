    ...
    public int getSize(
        ...
        if (layouts.size() > 0) {
            ...
            if (fm != null) {
                ...
                // 垂直偏移修复：原实现把 baseline 放在行盒底部（ascent=-(max+padding), descent=0），
                // 与相邻普通文本行基线对齐时，表格内容（画在行盒顶部）整体偏离文本基线，
                // 表现为表格内文字相对上下文垂直错位。
                // 修正：行盒总高不变，baseline 置于行盒垂直中线（上下行间距对称）。
                final int total = max + padding;
                final int half = total / 2;
                fm.top = -half;
                fm.bottom = total - half;
                fm.ascent = -half;
                fm.descent = 0;
            }
        }
    ...
    public void draw(
        ...
        for (int i = 0; i < size; i++) {
            ...
            try {
                ...
                // 垂直偏移修复：内容在行盒内垂直居中（原 heightDiff = /4 偏上）
                canvas.translate(padding, (bottom - top - layout.getHeight()) / 2);
                layout.draw(canvas);
